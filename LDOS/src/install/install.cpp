...
#define		ZSTD_SUPPORT		0
#define		UPX_SUPPORT			0
...
static	char	sPackFirePath[ _MAX_PATH ];
static	char	sShrinklerPath[ _MAX_PATH ];
static	char	sPaq8xPath[ _MAX_PATH ];
static	char	sUpxPath[_MAX_PATH];
static	char	sZStdPath[_MAX_PATH];

bool	g_bPack = true;
bool	g_bShrinkler = false;
int		g_iCurrentAlign = 2;
bool	g_bLZMA = false;
bool	g_bBestPacking = false;
...
//	ADFSave(pOutFile, m_pBuffer, m_maxSize);
	...
	int cylinderCount = CYLINDER_MAX; // CK, we want ADF 901120 bytes
...
void	CScreen::Display(int offset)
{
	...
	printf("%16s%3s %6d %6d (%3d%%) [%s] Off:$%06x (%02d/%d/%02d:$%03x) (user arg=%d)",	fName,fExt,
											m_originalSize&0x7fffffff,
											m_packedSize,
											(m_packedSize*100)/(m_originalSize&0x7fffffff),
											typeToPackerName(GetType()),
											offset,
											iCyl,iSide,iSector,offset&511,m_arg);
	...
	*cmd = 0;
	...
	switch (iPackingMethod)
	{
		...
		case PACK_ARJ4:
			...
			break;
		case PACKFIRE_TINY:
					sprintf_s(cmd, CMDLEN, "%s -b -t %s %s", sPackFirePath, pSource, sArchive);
					break;
		case PACK_LZ4:
			sprintf_s(cmd, CMDLEN, "%s -9 --no-frame-crc %s %s", sLZ4Path, pSource, sArchive);
			break;

		case PACKFIRE_LZMA:
				sprintf_s(cmd, CMDLEN, "%s -b -l %s %s", sPackFirePath, pSource, sArchive);
				break;
...
bool	CScreen::LoadScreen(const char *sFname,Type type, u16 arg, char* sRoot)
{
	...
	{
		mfile_t* pArj7 = NULL;
		mfile_t* pPackFire = NULL;

		unsigned long iOriginalSize = pOriginal->size;
		fileFree( pOriginal );

		if (packingMethod & ARJ7_METHOD)
			pArj7 = PackFile(pFilename, ARJ7_METHOD);

		if (packingMethod & PACK_ARJ4)
			pArj7 = PackFile(pFilename, PACK_ARJ4);

		if ( packingMethod & PACK_LZ4 )
			pPackFire = PackFile( pFilename, PACK_LZ4 );

		bool bPackFireTiny = false;
		mfile_t* pBest = pArj7;
		if ( pBest )
		{
			if (pPackFire)
			{
				if ( pPackFire->size < GetARJSize( pArj7->pData ) )
				{
					pBest = pPackFire;
					bPackFireTiny = true;
				}
		...
		if ((0x60 == pBest->pData[0]) && (0xea == pBest->pData[1]))
		{
			assert( !bPackFireTiny );
			// arj file ( http://datacompression.info/ArchiveFormats/arj.txt )
			u8 *pData = arjSkipHeader(pBest->pData);
			u8 *pData2 = NULL;
			if (pData)
				pData2 = arjSkipHeader(pData);
		...
		{
			assert( bPackFireTiny );
			m_packedSize = (pBest->size + 1) & (-2);
			m_originalSize = iOriginalSize;
			m_pBuffer = (u8*)malloc(m_packedSize);
			memset(m_pBuffer,0,m_packedSize);
			memcpy(m_pBuffer,pBest->pData,m_packedSize);		// taille + crc16
		}
...
int	main(int _argc, char *_argv[])
{
	...
	_makepath_s( sPackFirePath, _MAX_PATH, sDrive, sDir, "packfire", "exe");
	_makepath_s( sShrinklerPath, _MAX_PATH, sDrive, sDir, "shrinkler", "exe");
	_makepath_s( sPaq8xPath, _MAX_PATH, sDrive, sDir, "paq8px_v69", "exe");
