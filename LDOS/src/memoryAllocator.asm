...
batchAllocator:
		...
		; first total count each kind of mem
			...
			movea.l	a0,a1
.count:		move.l	(a1)+,d0
			bmi.s	.next ; -2 stop the loop
			lea		_chipInfo(pc),a2
			btst	#LDOS_MEM_ANY_RAM_BIT,d0 ; Flag 0 ?
			beq.s	.noAny1
			lea		_fastInfo(pc),a2
...
; input: d0: size (multiple of 8 bytes)
;		a1: address
fastClear:	movem.l	d0-d1/a1,-(a7)
			moveq	#0,d1
.loop:		move.l	d1,(a1)+
			move.l	d1,(a1)+
			subq.l	#8,d0
...
; a0: mem table
; d0: base ad
; d1.l: total size (byte)
initMemTable:
		movea.l	a0,a1
		move.l	d0,(a1)+
		;move.w	#MEMPAGE_COUNT/4-1,d0
        move.w	#MEMPAGE_COUNT-1,d0 ; Each block of 4Kb
.clr:	
        clr.b	(a1) ; Block is free
        cmp.l #0,d1
        bgt .freespace ; jump if block is free
        move.b #MEMLABEL_SYSTEM,(a1) ; Tagg this block as not free
.freespace:   
        add.l #1,a1
        sub.l #MEMPAGE_SIZE,d1 ; Remove 4kb page
		dbf		d0,.clr
		rts
...
; d0: size
; a0: memtable
; return d0 : -1 if alloc failed and adress if success
allocMemoryExt:
		addi.l	#MEMPAGE_SIZE-1,d0 ; 4Kb
		moveq	#MEMPAGE_SIZE_BIT,d2 ; 12
		lsr.l	d2,d0				; consecutive page count required ( 12KB, will give 3 )
		moveq	#0,d2				
		moveq	#0,d3
		move.l	(a0)+,d4			; base ad
		move.l	a0,a1
		move.w	#MEMPAGE_COUNT-1,d3 ; 128 blocks
...
allocPersistentChip:
            ...
            ; CK: Remove all system block (means not extra mem)
.removesystem:
            cmp.b #MEMLABEL_SYSTEM,-1(a1)
            bne .search
            sub.l #1,a1
            bra .removesystem
...
; -------------------------------------------
; Un mark = Free memory from mem label
; This do not erase the content
; d0.b: memory label to release		
unmarkMemLabel:
		movem.l	d0-a6,-(a7)
		lea		chipMemTable(pc),a0
