...
.already:
		bsr		runLoadedFile
		bra.s	mainDemoLoop ; [CK] No effect looping

; ----------------------------------------------------		

kernelLibrary:
			bra.w	userLoadNextFile ; 0*4 (loa music and exe)
			bra.w	vSync ; 1*4
			bra.w	installVBlank ; 2*4
			bra.w	batchAllocator ; 3*4
			bra.w	installCopperList ; 4*4
			bra.w	lz4_frame_depack
			bra.w	getFrameCounter
			bra.w	assertVector
			bra.w	musicStart
			bra.w	musicPlayer+12			; music get info
			bra.w	musicStop			; 10*4	; music stop (with fade out)
			bra.w	isDisk2Inserted
			bra.w	persistentAlloc
			bra.w	persistentGet
			bra.w	persistentTrash
			bra.w	cia50HzInstall ; 15*4
			bra.w	freeMemoryArea ; 16*4
			bra.w	musicPlayer+20 ; 17*4 ; music set position
			bra.w   musicreloc     ; 18*4 ; [CK] Only copy and init music.
			bra.w   loadFileMusic  ; 19*4 ; [CK] Only load music file. Do is number of file. 1 to 8
            bra.w   getMemBlocksTables ; 20*4 ; [CK] d0 = chip, d1 = fast
            bra.w   loadFileData   ; 21*4 [CK]
            bra.w   FreememDATA    ; 22*4
            bra.w   musicPlayer+28 ; 23*4 [CK] Music Get Trigger
            bra.w   FreememMUSIC   ; 24*4 [CK] Free music data
            
            ;bra.w   musicresume     ; 22*2 [CK] Pause/unpause music
...
; ----------------------------------------------------		
; [CK] Reloc and init module (free previous one) Must be preloaded		
musicreloc:
			;move.b #$a0,$106 ; Debug

			moveq	#MEMLABEL_PRECACHED_FX,d0 ; Free labels but we depack and use it in relocP61
			bsr		unmarkMemLabel				
			...
			;lea		currentFile(pc),a0 ; inc current file in case the next FX do a PRELOAD command
			;addq.w	#1,(a0)	
			
			;move.b #$af,$106 ; Debug
			rts
; ----------------------------------------------------	
; [CK] Pause / Unpause module	 
; For pause, just call "musicStop"          
;musicresume:
;
;
;            ;lea bMusicPlay,a0 ; w
;            ;lea bMusicPlayBeforePause,a1 ; w
;            ;lea MusicVolumeBeforePause,a2 ; w
;
;            ; Playing ?
;            ;cmp.w #0,(a0)
;            ;beq .notplaying
;            ; Playing, then we can PAUSE
;            ; -- Pause, volume to zero
;            ;move.w (a0),(a1) ; save play flag
;            bsr musicStop ; ask fade
;                ;move.w #0,(a0) ; clear play flag
;            ;bsr		musicPlayer+24			; Get volume in d0
;            ;move.w d0,(a2) ; save volume
;                ;move.w	#0,d0 ; set volume to zero
;                ;bsr		musicPlayer+16			; music set volume	
;            bra .end
;
;            ; If module not playing and was not playing before, then nothing to do
;.notplaying:
;            ; was playing before ?
;            cmp.w #0,(a1)
;            beq .end ; was  not playing before so do nothing
;            ; -- Unpause
;            move.w (a1),(a0) ; restore play flag
;            move.w #0,(a1) ; clear saved play flag
;			;move.w	(a2),d0 ; get saved volume
;			;bsr		musicPlayer+16			; restore music volume
;.end:
;            rts

; ----------------------------------------------------	
; Send back adresses of the two mem block tables (chip and fast)
getMemBlocksTables:
            lea	chipMemTable(pc),a0
            add.l #4,a0
            move.l a0,d0
...
musicStart:
			...
			move.w	#-1,(a0)
            ; request volume to max
            move.w #1,reqVolumeRestore
.skip:		rts
...
nextEXEDoAlloc:
        ...
        ; -- bit 0 means "music". Try to directly load in CHIP if music file
		btst	#0,(nextFx+m_flags+1)(pc)				
		beq.s	.normal
		move.l	d0,-(a7)
		bsr		allocChipMem
		tst.l	d0
...
nextDATADoAlloc:
		...
		move.w	12(a5),m_flags(a6)		; file entry flags
		move.w	14(a5),m_arg(a6)		; user arg
		move.b	#MEMLABEL_DATA,(SVAR_CURRENT_MEMLABEL).w
		move.l	d1,d0				; unpacked size
		addi.l	#DEPACK_IN_PLACE_MARGIN,d0
		bsr		allocAnyMem ; We want fast mem
.next:	lea		nextEXEDepacked(pc),a0
		move.l	d0,(a0)
		movem.l	(a7),d0-d1				;		
		addi.l	#DEPACK_IN_PLACE_MARGIN,d1
...
.infloop:	cmp.w	fatSize(pc),d0		; special case to test each FX. when we reach end of disk, loop here
			...
			move.l	d0,m_size(a6)
            ;move.l  d0,$100 ; Debug. Size. This is ok.        
			...
			bsr		nextEXEDoAlloc  ; tagged MEMLABEL_PRECACHED_FX
...
;-----------------------------------------------------------------		
; [CK]
; d0: file id ( script.txt order, start at 0 )	
; Will alloc place.
; return a0 memory
loadFileDATA_doloading:
            lsl.w	#4,d0				; FAT entry is 4 DWORDS: Floppy disk offset, Packed size, Unpacked size, user Arg
.infloop:	
            cmp.w	fatSize(pc),d0		; special case to test each FX. when we reach end of disk, loop here
            beq.s	.infloop
            lea	directory(pc),a5
            add.w	d0,a5
            lea		nextFx(pc),a6
            ...
            bsr		nextDATADoAlloc  ; tagged MEMLABEL_DATA
            
            move.l	nextEXEDepacked(pc),m_ad(a6) ; Depack of data adress
            move.l	nextEXEPacked(pc),a0
            movem.w	(a7)+,d0-d1
            pea		(a0)			; packed data load ad
            ; start trackloader !!
            ...
            ; run the depacker (packed data are loading async)
            bsr		mainThreadDepack
            move.l	m_ad(a6),a0 ; a0 contain depack adress
            rts
...
loadNextFile:
		...
		; Alloc trackloading buffers ( MFM and ARJ7 depacking buffer)
			...
			move.l	#MFM_DMA_SIZE,(a0)+ ; Chip ?
			move.l	#MFM_DMA_SIZE,(a0)+ ; Chip ?
			move.l	#13320 | LDOS_MEM_ANY_RAM,(a0)+ ; Fast
			lea		nextEXEAllocs(pc),a0
			bsr		batchAllocator
...
;-----------------------------------------------------------------	
; d0 id of file	(see list of file on disk, first is 0)
loadFileMusic:
    ;move.b  #$ff,$100 ; Debug
	lea     currentFile(pc),a0
	move.w  d0,(a0)
	...
	move.w	currentFile(pc),d0
	;move.b  d0,$105 ; Debug
	bsr		loadFile ; tagged MEMLABEL_PRECACHED_FX
	; music file, store pointers for next run
	movem.l	nextFx(pc),d0-d2		; m_ad, m_size, m_arg
	lea		nextMusic(pc),a0
	movem.l	d0-d2,(a0)
...
loadFileData:
	...
	move.w	currentFile(pc),d0
	;move.b  d0,$105 ; Debug
	bsr		loadFileDATA_doloading ; tagged MEMLABEL_DATA
	; free tackloading buffers
	moveq	#MEMLABEL_TRACKLOAD,d0
	bsr		freeMemLabel
    ; Return back location and size
 	lea		nextFx(pc),a6
    move.l  m_ad(a6),d0
    move.l  m_size(a6),d1
	rts
...
FreememDATA: 
    ; Free DATA memory (file loaded)
    moveq	#MEMLABEL_DATA,d0
	bsr		freeMemLabel
    rts
...
ldos50Hz:	
            ...
            ; -- Request volume max ?
            cmp.w #1,reqVolumeRestore
            bne .norequestmaxvolume
            move.w #0,reqVolumeRestore
			move.w	#64,d0
			bsr		musicPlayer+16			; music set volume				
.norequestmaxvolume:            
			; -- handle fade-out -----------------
...
systemChipBase:		dc.l	$108			; this is the first CHIP alloc by kernel, so if it fall to $0, keep $100 space for cpu vectors
...
sectorOffset:		ds.w	1
pModule:			dc.l	0
bMusicPlay:			dc.w	0 ; 0 = not playing, not 0 = playing
;bMusicPlayBeforePause: dc.w 0  ; [CK]
;MusicVolumeBeforePause: dc.w 0 ; [CK]
fadeOutStep:		dc.w	0
fadeOutValue:		dc.w	0
reqVolumeRestore:	dc.w	0 ; [CK] request to restore max volume
clockTick:			dc.l	0
startupFade:		dc.w	$fff
...
nextEXEAllocs:
pMFMRawBuffer1:		dc.l	0	;MFM_DMA_SIZE 13888 bytes ; Chip
pMFMRawBuffer2:		dc.l	0	;MFM_DMA_SIZE 13888 bytes ; Chip
